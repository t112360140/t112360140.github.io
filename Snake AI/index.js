...
async function TrainModel(model,config){
    ...
    let replay_mem_size = config["replay_mem_size"]!=null?config["replay_mem_size"]:1000;
    let minibatch_size = config["minibatch_size"]!=null?config["minibatch_size"]:1000;
    ...
    let replay_start_size = config["replay_start_size"]!=null?config["replay_start_size"]:1000;
    ...
    const BMax_add=config["BMax_add"]!=null?config["BMax_add"]:1;

    let agent =new DQN(5, discount, replay_mem_size, minibatch_size, epsilon,epsilon_stop_episode, epsilon_min, learning_rate,'meanSquaredError',tf.train.adam(learning_rate), hidden_dims, activations, replay_start_size, model);
    // let agent =new DQN(5, discount, epsilon,epsilon_stop_episode, epsilon_min, learning_rate,'meanSquaredError',tf.train.adam(learning_rate), hidden_dims, activations, model);
    ...
    stopTrain=false;
    saveModel=false;
    let BMax=config["BMax"]!=null?config["BMax"]:30;
    ...
    for(let episode=0;episode<episodes;episode++){
        ...
        while(!done&&(!max_steps||step<max_steps)){
            ...
            if(temp[0]==undefined){
                ...
                agent.update_replay_memory(current_state, 0, current_state, -50, done);
                // await agent.train(current_state,current_state,-10,epochs);
                break;
            }
            let best_index=0;
            let best_action=0;
            ...
            if(!done){
                ...
                best_index=best_state[1];
                best_action = next_states[best_index][0];

                const ret=env.play_game(best_action);
                done=(ret["status"]=='Game Over');
                reward=(1*(Bstep/BMax)+ret["score"]*10);
                if(ret["score"]>0){
                    Bstep=BMax;
                    BMax+=BMax_add;
                }
...
class DQN {
    constructor(state_size = 5, discount = 0.98, replay_mem_size = 20000,
...
/**
class DQN {
    constructor(state_size = 5, discount = 0.98, epsilon = 1,
        epsilon_stop_episode = 1500, epsilon_min = 1e-3,
        learning_rate = 1e-3, loss = 'meanSquaredError',
        optimizer = tf.train.adam(1e-3), hidden_dims = [64, 64],
...
class autoPlay{
    ...
    best_state(states){
        if(this.useAI){
            ...
            let best_index=0;
            for(let i=0;i<states.length;i++){
                if(max_value<(states[i][2]-states[i][1]-states[i][4])/states[i][3]){
                    max_value=(states[i][2]-states[i][1]-states[i][4]/states[i][3]);
                    best_index=i;
                }
            }
